...
def rolling_mean(series, window=4, plot_intervals=False, scale=1.96, plot_anomalies=False, figsize=(20,5), get_anomalies=False, plot=True):
...
def check_symmetry(ax, dff, rec_1, rec_2, tx1, tx2, plot=True, hist=False, resample='12H'):
    '''
    ...
    received = dff[dff['Receiver'] == rec_1].resample(resample).mean()[tx1]
    transmitted = dff[dff['Receiver'] == rec_2].resample(resample).mean()[tx2]
...
def create_labels(dff, anomalies_timestamps, transmitter='0x0057FE05', Receiver='00.57.FE.0F',
                  init_date='2017-12-05', end_date='2019-02-20', resample='5Min', label='SDR'):
    '''
    Example:
    
    data = create_labels(dff, radio_05_from_0F)
    anomalies_df = data[data['Label'] =='SDR']
    '''
    
    data = dff[dff['Receiver'] == Receiver][[transmitter]][init_date:end_date].resample(resample).mean().interpolate()
    data = data.drop(data.loc[data[transmitter] > -15].index, axis=0)
    
    data['Label'] = 'N'
    
    for ii in np.arange(0,len(anomalies_timestamps),2):
        data.loc[anomalies_timestamps[ii] : anomalies_timestamps[ii+1]]['Label'] = label
    
    #data.to_csv('../Labeled/radio_05_from_0F.csv')
...
def create_segments_and_labels(df, time_steps, step, label_name, features):
    ...
    for i in range(0, len(df) - time_steps, step):
        serie_1 = df[features[0]].values[i: i + time_steps]
        serie_2 = df[features[1]].values[i: i + time_steps]
        # Retrieve the most often used label in this segment
        label = stats.mode(df[label_name][i: i + time_steps])[0][0]
        segments.append([serie_1, serie_2])
        labels.append(label)
...
def plot_scaleogram(signal, resample_time = 10, wavelet='cmor3-1.5', log=False,
                    scales=np.arange(1, 256), xlabel='Days', ylabel='Period (Hour)', 
                   figsize=(21,7), xtickslabels=True, plot_signal=True, monthly=False,
                   lineplot_size=(22,3), ax1=0):
    # take a week
    #ax1 is the axis to plot otherwise creates a figure
    
    time = np.arange(len(signal))
    day_scale = 24*60/resample_time
    hour_scale = day_scale/24
    # vectorized numpy function to cast an integer
    fun = np.vectorize(custom_to_int)  
    if plot_signal:
        ax1 = 0
        fig, ax1 = plt.subplots(1, 1, figsize=lineplot_size)
        sns.lineplot(data=signal, ax=ax1)
        try:
            plt.xlim(signal.index[0], signal.index[-1])
        except:
            pass
        plt.ylabel('{}\n'.format(signal.name))
        plt.tight_layout()
    ...
    if log:
        ax1 = scg.cws(time, signal, scales=scales, wavelet=wavelet,
                  ax=ax1, cmap="jet", ylabel="Period [minutes]", xlabel="Time [minutes]",
                  title='Scaleogram with linear period yscale', coikw={'alpha':0.5}, yscale='log')
        #ax1.set_yticks(ax1.get_yticks()/hour_scale)
        ax1.set_yticklabels(fun(ax1.get_yticks()/hour_scale))
        ax1.set_ylabel('Period (Hour)')
    else:
        ax1 = scg.cws(time, signal, scales=scales, wavelet=wavelet,
                  ax=ax1, cmap="jet", ylabel="Period [minutes]", xlabel="Time [minutes]",
                  title='Scaleogram with linear period yscale', coikw={'alpha':0.5})

        ax1.set_yticks(np.arange(ax1.get_yticks().min(), ax1.get_yticks().max(), 12))
        ax1.set_yticklabels(fun(ax1.get_yticks()/hour_scale))
        ax1.set_ylabel('Period (Hour)')
    if xtickslabels:
        # set half of scale
        x_scale = np.arange(ax1.get_xticks().min(), ax1.get_xticks()[-2]+(ax1.get_xticks()[-1] - ax1.get_xticks()[-2])/2, day_scale)
        ax1.set_xticks(x_scale)
        # convert to days
        ax1.set_xticklabels(fun(np.round(ax1.get_xticks()/day_scale, 0)))
        ax1.set_xlabel('Days')
    if monthly:
        # set half of scale
        x_scale = np.arange(ax1.get_xticks().min(), ax1.get_xticks()[-2]+(ax1.get_xticks()[-1] - ax1.get_xticks()[-2])/2, day_scale*30)
        ax1.set_xticks(x_scale)
        # convert to days
        ax1.set_xticklabels(fun(np.round(ax1.get_xticks()/day_scale, 0)))
        ax1.set_xlabel('Days')
